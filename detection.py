...
DOT_SAVE_VIS = False
DOT_VERBOSE = False
DOT_PROGRESS_EVERY = 100
DELETE_ROIS_AFTER_BINARY = True
...
def run_dot_detection(iise_dir, img_dir, out_dir):
    """Run improved dot detection on defect images and write all_dots.csv."""
    import csv as _csv
    sys.path.insert(0, iise_dir)
    from detect_dots import detect_image_improved

    os.makedirs(out_dir, exist_ok=True)
    images = sorted(Path(img_dir).glob("*.jpg"))
    print(f"    Detecting dots in {len(images)} images ...")
    ...
    for i, img_path in enumerate(images, 1):
        ...
        rows = detect_image_improved(gray, img_path.name)
        all_rows.extend(rows)
        if DOT_PROGRESS_EVERY and (i % DOT_PROGRESS_EVERY == 0 or i == len(images)):
            print(f"    Processed {i}/{len(images)} images")

    csv_path = os.path.join(out_dir, "all_dots.csv")
    with open(csv_path, 'w', newline='') as f:
        w = _csv.writer(f)
        w.writerow(['img', 'x', 'y', 'sigma', 'circle_id'])
        for d in all_rows:
            w.writerow([d['img'], d['x'], d['y'], f"{d['sigma']:.2f}", d['circle_id']])
    print(f"    Saved {len(all_rows)} dot detections → {csv_path}")
    return True


def cleanup_roi_dir(roi_dir):
    """Delete generated ROI crops once binary inference has finished."""
    if os.path.isdir(roi_dir):
        shutil.rmtree(roi_dir)
        print(f"    Deleted ROI directory: {roi_dir}")
...
def main():
    ...
    if DELETE_ROIS_AFTER_BINARY:
        cleanup_roi_dir(ROI_DIR)
    ...
    if not run_dt_classification(IISE_DIR, TEMP_DEFECT_DIR, dots_csv, CLASSIFY_OUT, DT1_MODE):
        sys.exit(1)
